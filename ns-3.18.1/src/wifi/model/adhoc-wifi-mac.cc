...
AdhocWifiMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr)
{
  ...
  if (hdr->IsFeedback ())
  {
		  ...
		  m_rxInfoSet.perRate = fbhdr.GetPerRate();
			...
			NS_LOG_INFO ("[rx feedback packet]" 
					<< " perMCS0: " << (int)m_rxInfoSet.perRate.perMCS0 << " perMCS1: " << (int)m_rxInfoSet.perRate.perMCS1 
					<< " perMCS2: " << (int)m_rxInfoSet.perRate.perMCS2 << " perMCS3: " << (int)m_rxInfoSet.perRate.perMCS3 
					<< " perMCS4: " << (int)m_rxInfoSet.perRate.perMCS4	<< " perMCS5: " << (int)m_rxInfoSet.perRate.perMCS5 
					<< " perMCS6: " << (int)m_rxInfoSet.perRate.perMCS6	<< " perMCS7: " << (int)m_rxInfoSet.perRate.perMCS7);
  ...
  if (hdr->IsData ())
  {
		  ...
		  else if (to.IsGroup ())
		  {
				if (m_initialize == false)
				{
					...
					rand -> GetValue(0, m_feedbackPeriod);
...
AdhocWifiMac::SendFeedback ()
{
  ...
  if (m_fbtype != 4){
	...
	per_rate.perMCS0 = (uint16_t)(1000*m_tableManager->GetPdr(0, (uint32_t)snr));
	per_rate.perMCS1 = (uint16_t)(1000*m_tableManager->GetPdr(1, (uint32_t)snr));
	per_rate.perMCS2 = (uint16_t)(1000*m_tableManager->GetPdr(2, (uint32_t)snr));
	per_rate.perMCS3 = (uint16_t)(1000*m_tableManager->GetPdr(3, (uint32_t)snr));
	per_rate.perMCS4 = (uint16_t)(1000*m_tableManager->GetPdr(4, (uint32_t)snr));
	per_rate.perMCS5 = (uint16_t)(1000*m_tableManager->GetPdr(5, (uint32_t)snr));
	per_rate.perMCS6 = (uint16_t)(1000*m_tableManager->GetPdr(6, (uint32_t)snr));
	per_rate.perMCS7 = (uint16_t)(1000*m_tableManager->GetPdr(7, (uint32_t)snr));
...
// jychoi
uint8_t
AdhocWifiMac::GroupRateAdaptation (void)
{
	// Add Basic Mode
	...
	else
	{
		...
		else
		{
			...
			} // find m_minSnr over 3 dB

			// Set perOfMcs vector with min per
			SetMinPerOfMcs ();
			...
			//{
				...
				for (uint32_t k = 0; k < NBasicMode; k++)
				{
					...
					else if(m_ratype == 1)
					{
						...
						if (Per == 1)
							m_GroupTxMode = m_stationManager->GetBasicMode (0);
						else if (Per > 1)
							NS_LOG_ERROR("Never happen");
						
						m_burstsize = KtoNTable[m_k-1][(uint8_t)(100-Per*100)];
					}//m_ratype == 1
					...
					else if (m_ratype == 2)
					{
						...
						m_burstsize = KtoNTable[m_k-1][(uint32_t)(100-Per*100)];
					}//m_ratype==2
			...
			else
			{
				...
				m_GroupTxMcs = 0;
				m_burstsize = 30; // because m_minSnr is under 3 dB
			} // m_minSnr < 3
			*/
		} // vsize != 0
	}// m_setRobust == false
