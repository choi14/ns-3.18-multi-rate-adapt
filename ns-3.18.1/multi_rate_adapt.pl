...
foreach $bound (20){
	foreach $doppler (0.05, 0.1){
		foreach $n_node (1, 5, 10){
			$output_filename1="storage_results/output2_151109/result_avg-"."$doppler"."-"."$n_node"."-"."$bound".".txt";
			open(OUT1, ">$output_filename1") ||die "Failed opening.\n";
			$output_filename2="storage_results/output2_151109/result_air-"."$doppler"."-"."$n_node"."-"."$bound".".txt";
			open(OUT2, ">$output_filename2") ||die "Failed opening.\n";
			$output_filename3="storage_results/output2_151109/result_node-"."$doppler"."-"."$n_node"."-"."$bound".".txt";
			...
			foreach $period (100, 200, 500, 1000){
				...
				for ($type = 0; $type < 9; $type++){
					...
					$avgPer = sprintf"%.3f",$avgPer/100;
					$avgAir = sprintf"%.3f",$avgAir/100;
					$avgNr = sprintf"%.3f",$avgNr/100;
					...
					$stddev[0] = sprintf"%.3f",sqrt($stddev[0]/100);
					$stddev[1] = sprintf"%.3f",sqrt($stddev[1]/100);
					$stddev[2] = sprintf"%.3f",sqrt($stddev[2]/100);
