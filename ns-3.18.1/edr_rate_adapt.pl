...
foreach my $linearTime (2000){
	foreach my $blocksize (1, 5, 10){
	foreach my $edrType (1, 2){
		foreach my $numNode (10){
			foreach my $period (1000){
				foreach my $doppler (0.1){
					foreach my $bound (10, 20){
						...
						foreach my $eta (1){
							...
							print OUT1 "$eta $per_a[0] $per_s[0] $per_a[1] $per_s[1]\n";
							print OUT2 "$eta $air_a[0] $air_s[0] $air_a[1] $air_s[1]\n";
							print OUT3 "$eta $node_a[0] $node_s[0] $node_a[1] $node_s[1]\n";

							for($i = 2; $i < 6; $i++)
							{
								print OUT1 "$i $per_a[0] $per_s[0] $per_a[1] $per_s[1]\n";
								print OUT2 "$i $air_a[0] $air_s[0] $air_a[1] $air_s[1]\n";
								print OUT3 "$i $node_a[0] $node_s[0] $node_a[1] $node_s[1]\n";
							}

							print "---------------------------------------------------------------------------\n";
							print "$eta $per_a[0] $per_s[0] $per_a[1] $per_s[1]\n";
							print "$eta $air_a[0] $air_s[0] $air_a[1] $air_s[1]\n";
							print "$eta $node_a[0] $node_s[0] $node_a[1] $node_s[1]\n";
							print "---------------------------------------------------------------------------\n";
