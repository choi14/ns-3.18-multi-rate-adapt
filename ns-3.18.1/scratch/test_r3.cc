...
#include "ns3/online-table-manager.h"
...
main (int argc, char *argv[])
{
...
#ifdef MAKINGTABLE
	Config::SetDefault ("ns3::AdhocWifiMac::IsMakingTable", BooleanValue (true));
#else
	Config::SetDefault ("ns3::AdhocWifiMac::IsMakingTable", BooleanValue (false));
#endif
  ...
  Ipv4StaticRoutingHelper multicast;
	...
	if(!fin.good())
		NS_LOG_UNCOND("File open failed " << in_filename.str());
	...
	for(uint32_t i = 0; i < allDevice.GetN(); i++){
		...
		for(uint16_t mcs = 0; mcs < 8; mcs++){
			for(uint16_t rssi = 0; rssi < 50; rssi++){
				if(!fin.eof()){
					double pdr;
					fin >> pdr;
					tableManager->SetPdr(mcs, rssi, pdr);
				}
			}
		}
  ...
  Simulator::Run ();
	...
	switch (feedbackType)
	{
		...
		case 2:
			...
			break;
		case 3: 
			NS_LOG_UNCOND ("seed: " << seed << " feedbackPeriod: " << feedbackPeriod <<	" dopplerVelocity: " << dopplerVelocity << " feedbackType: " << feedbackType <<
					" eta: " << eta << " delta: " << delta << " rho: " << rho <<  " bound: " << bound << " rxNodeNum: " << rxNodeNum << " edrType: " << edrType << " blockSize: " << blockSize);
